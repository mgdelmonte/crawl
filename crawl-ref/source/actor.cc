...
{
    const char* acnames[] = {"none","normal","half","triple","proportional"};
    if( for_real )
        dprf(DIAG_COMBAT, "apply_ac %s ac=%d gdr=%d damage=%d max_damage=%d ac_rule=%s stab_bypass=%d", name(DESC_THE).c_str(), armour_class(), gdr_perc(), damage, max_damage, acnames[(int)ac_rule], stab_bypass);
    ...
    if( for_real )
        dprf(DIAG_COMBAT, "         %s saved=%d ac=%d gdr=%d after=%d", name(DESC_THE).c_str(), saved, ac, gdr, max(saved, min(gdr * max_damage / 100, ac / 2)));
